...
<div class="main-content">
    <div class="form-container">
        <h1 style="margin-bottom: 20px;">Edit Batch</h1>
        <?php if (isset($_SESSION['error'])): ?>
            <div class="alert alert-error"><?php echo htmlspecialchars($_SESSION['error']); unset($_SESSION['error']); ?></div>
        <?php endif; ?>

        <form method="POST" action="index.php?action=batch&sub=edit&id=<?php echo $batch->id; ?>">
            <div class="form-group">
                <label for="batch_no">Batch No *</label>
                <input type="text" id="batch_no" name="batch_no" required value="<?php echo htmlspecialchars($batch->batch_no ?? ''); ?>">
            </div>
            <div class="form-group">
                ...
                <select id="cid" name="cid" required>
                    ...
                    <?php foreach($courses as $course): ?>
                        <option value="<?php echo $course['id']; ?>" <?php echo ($batch->cid == $course['id'])?'selected':''; ?>>
            ...
            <div class="form-group">
                ...
                <select id="version_id" name="version_id">
                    ...
                    <?php foreach($versions as $version): ?>
                        <option value="<?php echo $version['id']; ?>" <?php echo ($batch->version_id == $version['id'])?'selected':''; ?>>
            ...
            <div class="form-group">
                ...
                <input type="date" id="start_date" name="start_date" value="<?php echo htmlspecialchars($batch->start_date ?? ''); ?>">
            </div>
            <div class="form-group">
                <label for="end_date">End Date</label>
                <input type="date" id="end_date" name="end_date" value="<?php echo htmlspecialchars($batch->end_date ?? ''); ?>">
            </div>
...
const initialVersionId = <?php echo $batch->version_id ?? 'null'; ?>;
...
// Load initial versions if course is already selected
if (initialCourseId) {
