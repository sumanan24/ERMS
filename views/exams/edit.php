...
<div class="main-content">
    <div class="form-container">
        <h1 style="margin-bottom: 20px;">Edit Exam Schedule</h1>
        <?php if (isset($_SESSION['error'])): ?>
            <div class="alert alert-error"><?php echo htmlspecialchars($_SESSION['error']); unset($_SESSION['error']); ?></div>
        <?php endif; ?>

        <form method="POST" action="index.php?action=exams&sub=edit&id=<?php echo $exam->id; ?>">
            <div class="form-group">
                <label for="exam_date">Exam Date *</label>
                <input type="date" id="exam_date" name="exam_date" required value="<?php echo htmlspecialchars($exam->exam_date ?? ''); ?>">
            </div>
            <div class="form-group">
                <label for="time_slot">Time Slot * (e.g., 01.00-4.00 or 9.00-12.00)</label>
                <input type="text" id="time_slot" name="time_slot" required placeholder="01.00-4.00" value="<?php echo htmlspecialchars($exam->time_slot ?? ''); ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="module_id" name="module_id" required>
                    ...
                    <?php foreach($modules as $module): ?>
                        <option value="<?php echo $module['id']; ?>" <?php echo ($exam->module_id == $module['id'])?'selected':''; ?>>
                            <?php echo htmlspecialchars($module['mcode'] . ' - ' . $module['mname']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <input type="text" id="location" name="location" required placeholder="hall 01" value="<?php echo htmlspecialchars($exam->location ?? ''); ?>">
            </div>
            <div class="form-group">
                ...
                <input type="number" id="assessment_percentage" name="assessment_percentage" step="0.01" min="0" max="100" 
                       required placeholder="e.g., 30" value="<?php echo htmlspecialchars($exam->assessment_percentage ?? 0); ?>" 
            ...
            <div class="form-group">
                ...
                <input type="number" id="final_exam_percentage" name="final_exam_percentage" step="0.01" min="0" max="100" 
                       required placeholder="e.g., 70" value="<?php echo htmlspecialchars($exam->final_exam_percentage ?? 0); ?>" 
                       oninput="validatePercentages()">
...
const initialModuleId = <?php echo $exam->module_id ?? 'null'; ?>;
...
// Load initial modules if course is already selected
if (initialCourseId) {
